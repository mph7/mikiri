...
import { Mail, Lock, LogIn as LogInIcon, EyeOff, Eye } from 'lucide-react'
...
const Login = ({ onSubmit, onSwitchMode }) => {
    ...
    return (
        <div className='max-w-md bg-white shadow-lg border border-red-100 rounded-xl p-12
        flex flex-col items-center justify-center'>
            <div className='text-center mb-6'>
                <div className='w-16 h-16 flex items-center justify-center text-red-600 rounded-full bg-gradient-to-br
          from-red-500 to-red-800 mx-auto'>
            ...
            {message.text ? <div className={message.type === 'success' ? MESSAGE_SUCCESS : MESSAGE_ERROR}>
                {message.text}
            </div> : ''}

            <form onSubmit={handleSubmit} className='gap-2 flex flex-col'>
                {FIELDS.map(({ name, type, placeholder, icon: Icon }) => (
                    <div className='flex border border-red-100 px-3 py-2.5 rounded-lg mb-2 items-center
                    focus-within:ring-red-500 focus-within:ring-2 focus-within:border-red-500 
                    transition-all duration-200'>
                        <Icon className='text-red-500 w-5 h-5 mr-2' />
                        <input type={type !== 'password' ? type : showPassword === true ? 'text' : 'password'}
                ...
                <button type='submit' className='bg-gradient-to-r from-red-500 to-red-900 w-full text-white py-2.5
                rounded-lg cursor-pointer mt-2 flex gap-2 items-center justify-center text-sm font-semibold
                hover:shadow-md transition-all duration-200' disabled={loading}>
                    {loading ? 'Loging In...' : <><LogInIcon className='w-5 h-5' /> Login</>}
                </button>
            </form>
            <p className='text-sm mt-6 text-gray-600 text-center'>Doesn't have an account yet? {' '}
